...
namespace DataLogic
{
    public class LoadSubTables
    {
        ...
        public static void BuildInsertStmt()
        {
            Log.Debug("Dbconn called and created for BuildInsertStmt()");
            using (var conn = new SqlConnection(ConfigurationManager.ConnectionStrings["Dbconn"].ConnectionString))

            using (var cmd = conn.CreateCommand())
            {
                var sql = Settings.Default.SqlInsertNoteData;
                cmd.CommandText = sql;
                ...
                cmd.Parameters.Add("@id", SqlDbType.VarChar).Value = NoteId;
                ...
                try
                {
                    try
                    {
                        ...
                        Log.Info("Inserting values into DataControl tables from LoadSubTables.BuildInsertStmt() for note inserts");
                    ...
                    catch (Exception e)
                    {
                        ...
                        var title = "Database Insertion Error";
                ...
                {
                    Log.Info("Closing SQL Connection for LoadSubTables.BuildInsertStmt()");
                }
            }
        }
    }
